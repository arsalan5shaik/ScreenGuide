# ScreenGuide â€” Feature Testing Guide
...
âœ… `ScreenGuide is running` toast notification appears  
...
âœ… ScreenGuide describes the current page/app  
...
2. Say **"ScreenGuide"** then immediately *"what time is it?"*

âœ… Blue buddy reacts after you say "ScreenGuide"  
âœ… ScreenGuide answers the question  
...
âœ… ScreenGuide says *"That's the Google search bar"* (or similar)  
...
âœ… ScreenGuide gives Step 1, ends with *"say 'next' when ready"*  
3. Say **"next"**  
...
âœ… ScreenGuide replays the last TTS without querying the LLM again  
...
âœ… ScreenGuide answers using the new provider  
...
1. Have a 3â€“4 question conversation with ScreenGuide
2. Say *"what did we cover today?"*

âœ… ScreenGuide summarises today's Q&A from the local journal  
...
âœ… ScreenGuide asks YOU a question about what's on screen  
âœ… Answer it â€” ScreenGuide evaluates in one sentence  
...
âœ… ScreenGuide runs OCR on the screenshot  
...
1. Ask a question where ScreenGuide would point at multiple things, e.g.:
...
4. Open `%LOCALAPPDATA%\ScreenGuide\recordings\`  
âœ… MP4 file exists  
...
3. Tray â†’ **Workflow Capture â†’ Stop + send to ScreenGuide**
4. Ask *"what did I just do?"*

âœ… ScreenGuide narrates your workflow step by step  
...
3. Ask ScreenGuide anything

âœ… ScreenGuide says it skipped the screenshot for privacy  
...
1. Ask ScreenGuide something in Chrome: *"what's on screen?"*
2. Switch to VS Code
3. Ask: *"what were we just talking about?"*

âœ… ScreenGuide has separate context â€” it won't mention the Chrome content  
...
1. Copy `skills/example_self_mode.py` to `~/.screenguide/skills/my_skill.py`
2. Restart ScreenGuide
...
âœ… Explorer opens `%LOCALAPPDATA%\ScreenGuide\`  
...
4. Say "ScreenGuide, where is [element]"  â†’ buddy flies to it
...
10. Tray â†’ Quit ScreenGuide               â†’ clean exit
```

All 10 steps passing = ScreenGuide is fully functional.
