...
## Hi, this is ScreenGuide


ScreenGuide is a little AI buddy that **lives next to your cursor**. Hold a hotkey, ask it something about your screen, and it talks back — pointing at buttons, walking you through steps, drawing arrows on your screen. Think of it as a patient tutor sitting beside you while you learn anything: video editing, coding, a new app, whatever.

No more Alt-Tab to ChatGPT. No more typing out descriptions of what's on your screen. Just hold **Ctrl + Win**, speak, and ScreenGuide handles the rest.
...
│                                             │
│          🔵◂ ← ScreenGuide, your blue buddy      │
│          (floats beside your real cursor)   │
│                                             │
│  ┌───────────────────────────┐              │
│  │  ScreenGuide  [Claude]     —   │  ← panel     │
│  │  ● Thinking…              │              │
│  │  "The search bar is       │              │
│  │   right here ↗"           │              │
...
- Say **"ScreenGuide"** for hands-free wake word
...
- **SM-2 spaced repetition** — ScreenGuide reminds you to review topics at optimal intervals (1 → 3 → 7 → 14 → 30 → 60 → 120 days)
- Say **"quiz me on what I should review"** → flashcard session from your journal
- Journal stored at `%LOCALAPPDATA%\ScreenGuide\journal.db`

### 📄 Document Context (Drag & Drop)
- Drag a **PDF, DOCX, TXT, MD, CSV, or code file** onto the ScreenGuide panel
- ScreenGuide reads it and uses it as context for your next questions
...
- When your query mentions "fine print", "small text", "read that", etc., ScreenGuide runs **Tesseract OCR** on the screenshot
...
| **Quiz Mode** | ScreenGuide asks *you* questions instead of answering; evaluates in one sentence |
...
- Saved to `%LOCALAPPDATA%\ScreenGuide\recordings\`

### 🖊️ Whiteboard Annotations
ScreenGuide can draw directly on your screen as it explains:
...
- Stop: **Tray → Workflow Capture → Stop + send to ScreenGuide**
- Ask **"What did I just do?"** → ScreenGuide narrates your workflow step-by-step
...
Create your own voice triggers by dropping in a Python file:

```python
# ~/.screenguide/skills/my_skill.py
import asyncio
...
Place skill files in `~/.screenguide/skills/` — ScreenGuide auto-loads them on startup.
...
```
Token cached at `%LOCALAPPDATA%\ScreenGuide\github_token.json`. Default model: `gpt-4o-mini` (free tier).

### 5. Run
```bash
python main.py
```
A blue dot appears in your system tray. ScreenGuide is now running.
...
ScreenGuide uses **two Ollama model slots**: a vision model for screen-aware questions (pointing, "what's on screen?") and a text model for Code Mode / journal Q&A. Switch between them anytime from the tray: **Tray → Ollama → Vision model / Text model**.
...
pyinstaller screenguide.spec --clean --noconfirm

# 3. Distribute
# → dist/ScreenGuide/  (entire folder)
# → or build Setup.exe with Inno Setup using installer.iss
```

`dist/ScreenGuide/ScreenGuide.exe` runs on any Windows machine without Python installed. Include a `.env` file next to the exe with your API keys, or set them as system environment variables.
...
├── screenguide.spec                  # PyInstaller build spec
...
| `SCREENGUIDE_HOTKEY` | `ctrl+win` | Global push-to-talk combo |
| `SCREENGUIDE_STT` | *(auto)* | Force STT: `deepgram` / `openai` / `whisper_cpp` / `faster_whisper` |
| `SCREENGUIDE_ACTIVE_LLM` | *(auto)* | Force LLM: `claude` / `openai` / `copilot` / `gemini` / `ollama` / `lmstudio` |
...
| `RESPONSE_LANGUAGE` | *(empty)* | Force ScreenGuide's reply language regardless of input; empty = auto-detect |
| `MIC_DEVICE_INDEX` | *(empty)* | Force a specific input device; empty = system default |
| `CUSTOM_INSTRUCTIONS` | *(built-in prompt)* | Full override of ScreenGuide's system prompt — also editable live via Tray |

---

## Troubleshooting

**ScreenGuide doesn't hear me**
...
**ScreenGuide mixes languages in its replies**
...
→ Check that `%LOCALAPPDATA%\ScreenGuide\github_token.json` exists
